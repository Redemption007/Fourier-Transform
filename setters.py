...
from variables import h, w, var, samples_var, frequency_var, signal_on_noise, verif_mod, error, verif_modif, show_error
from analyse import analyse

def change_state(widget=None, newstate=None, key=None):
    if key:
        var[key]['value'] = not var[key]['value']
    if newstate:
...
def set_amplitude(frame):
    amplitude = tkt.Frame(frame, width=w//2)

    label_amplitude = tkt.Label(amplitude, text = "A(t) = ", anchor=tkt.E)
    label_amplitude.grid(row=1, column=0)
    
    initial_amplitude = tkt.Entry(amplitude, name='initial_amp', width=4, textvariable=var["initial_amp"]["value"], validate='focusout', validatecommand=(verif_mod, '%W', '%P', '%s'), invalidcommand=(error, '%W', '%P', '%s'))
    initial_amplitude.grid(row=1, column=1)
    
    label_exp = tkt.Label(amplitude, text = "* exp( -")
    label_exp.grid(row=1, column=2)
    
    damping = tkt.Entry(amplitude, name='damping', width=4, textvariable=var["damping"]["value"], validate='focusout', validatecommand=(verif_mod, '%W', '%P', '%s'), invalidcommand=(error, '%W', '%P', '%s'))
...
def set_sinusoidale(frame):
    phase = tkt.Frame(frame, width=w//2)

    label_omega = tkt.Label(phase, text = "\u03c9 = ", anchor=tkt.E)
    label_omega.grid(row=0, column=0)
    
    omega = tkt.Entry(phase, name='w', width=4, textvariable=var["w"]["value"], validate='focusout', validatecommand=(verif_mod, '%W', '%P', '%s'), invalidcommand=(error, '%W', '%P', '%s'))
    omega.grid(row=0, column=1)
    
    phase.grid(column=0, row=2, padx=20, sticky=tkt.W)
    #
    dephasage = tkt.Frame(frame, width=w//2)
    
    label_phi = tkt.Label(dephasage, text = "\u03c6 = ", anchor=tkt.E)
    label_phi.grid(row=0, column=3)
    
    phi = tkt.Entry(dephasage, name='phi', width=4, textvariable=var["phi"]["value"], validate='focusout', validatecommand=(verif_mod, '%W', '%P', '%s'), invalidcommand=(error, '%W', '%P', '%s'))
...
def set_sampling(frame):
    samples = tkt.Frame(frame, width=w//2)

    label_samples = tkt.Label(samples, text = "Échantillonnage : ", anchor=tkt.E)
    label_samples.grid(row=0, column=0)
    sampling = tkt.Entry(samples, name='sampling', width= 4, textvariable=var["sampling"]["value"], validate='focusout', validatecommand=(verif_mod, '%W', '%P', '%s'), invalidcommand=(error, '%W', '%P', '%s'))
    sampling.grid(row=0, column=1)

    label_periods = tkt.Label(samples, text = "Périodes : ", anchor=tkt.E)
    label_periods.grid(row=1, column=0)
    periods = tkt.Entry(samples, name='periods', width= 4, textvariable=var["periods"]["value"], validate='focusout', validatecommand=(verif_mod, '%W', '%P', '%s'), invalidcommand=(error, '%W', '%P', '%s'))
...
def set_infos(frame):
    ...
    label_samples = tkt.Label(infos, textvariable = samples_var, anchor=tkt.W)
    label_samples.grid(row=0, column=0)
    
    label_freq = tkt.Label(infos, textvariable = frequency_var, anchor=tkt.W)
    label_freq.grid(row=1, column=0)

    label_ratio = tkt.Label(infos, textvariable = signal_on_noise, anchor=tkt.W)
...
def set_options(frame):
    ...
    random_num_entry = tkt.Entry(options_labelframe, name='random_nb', width=4, textvariable=var["random_nb"]["value"], state='disabled', validate='focusout', validatecommand=(verif_mod, '%W', '%P', '%s'), invalidcommand=(error, '%W', '%P', '%s'))
    ...
    periodics_freq_entry = tkt.Entry(options_labelframe, name='periodic_f', width=4, textvariable=var["periodic_f"]["value"], state='disabled', validate='focusout', validatecommand=(verif_mod, '%W', '%P', '%s'), invalidcommand=(error, '%W', '%P', '%s'))
    ...
    periodics_width_entry = tkt.Entry(options_labelframe, name='periodic_width', width=4, textvariable=var["periodic_width"]["value"], state='disabled', validate='focusout', validatecommand=(verif_mod, '%W', '%P', '%s'), invalidcommand=(error, '%W', '%P', '%s'))
    ...
    zero_pad_entry = tkt.Entry(options_labelframe, name='zp_nb', width=4, textvariable=var["zp_nb"]["value"], state='disabled', validate='focusout', validatecommand=(verif_mod, '%W', '%P', '%s'), invalidcommand=(error, '%W', '%P', '%s'))
...
def set_reexcitement(frame):
    ...
    randoms_frame = tkt.Frame(reexcitement_labelframe, width=w//2)
    ...
    randoms_entry = tkt.Entry(randoms_frame, name='reex_nb', width=4, textvariable=var["reex_nb"]["value"], state='disabled', validate='focusout', validatecommand=(verif_mod, '%W', '%P', '%s'), invalidcommand=(error, '%W', '%P', '%s'))
    randoms_entry.grid(column=1, row=0, sticky='W')

    sill_frame = tkt.Frame(reexcitement_labelframe, width=w//2)
    ...
    sill_entry = tkt.Entry(sill_frame, name='reex_sill', width=4, textvariable=var["reex_sill"]["value"], state='disabled', validate='focusout', validatecommand=(verif_mod, '%W', '%P', '%s'), invalidcommand=(error, '%W', '%P', '%s'))
    ...
    def radio_choice():
        ...
        if ch==1:
            ...
            var['reexcitement']['value'] = 'randoms'
        elif ch==2:
            change_state(sill_entry, newstate='normal')
            change_state(randoms_entry, newstate='disabled')
            var['reexcitement']['value'] = 'periodics'
        else:
            change_state(sill_entry, newstate='disabled')
            change_state(randoms_entry, newstate='disabled')
            var['reexcitement']['value'] = 'without'
        return
...
def set_buttons(settings, window, glob):
    controls = tkt.Frame(settings, width=w//4, height=30)
...
def set_settings(frame, window):
    settings = tkt.LabelFrame(frame, text="Réglages : ", height=h//2, width=w//2)
...
def set_graphs(frame, window):
    ...
    if W != '.' and W.find('canvas') == -1:
        if not verif_modif(W):
            return show_error(W)

    graphs = tkt.Frame(frame, width=w//2, height=h)
    graphs.grid(column=1, row=0, rowspan=2, sticky='EWNS')

    graphs.rowconfigure(0, minsize=h//2)
    graphs.rowconfigure(1, minsize=h//2)

    graph_1 = tkt.Frame(graphs, bg="#ff0000", width=w//2, height=h//2)
    graph_1.grid(column=0, row=0, sticky='EWNS')

    graph_2 = tkt.Frame(graphs, bg="#00ff00", width=w//2, height=h//2)
    graph_2.grid(column=0, row=1, sticky='EWNS')

    signal_graph = tkt.Frame(frame, bg="#ff0000", width=w//2, height=h//2)
...
def set_glob(window):
    ...
    glob = tkt.Frame(window, height=h, width=w)
    
    glob.columnconfigure(0, weight=1)
    glob.columnconfigure(1, weight=1)
    glob.rowconfigure(0, minsize=int(h*4/7))
    glob.rowconfigure(1, minsize=int(h*3/7))
